fun main() {
    ...
    testFirstTask(3.0, 5.2, 4.7)    // среднее - 4.7
    testFirstTask(2.7, 2.7, 3.4)    // нет среднего (есть равные)
    testFirstTask(1.5, 10.8, 5.6)   // среднее - 5.6
    testFirstTask(7.5, 3.2, 9.8)    // среднее - 7.5
    testFirstTask(4.5, 4.5, 4.5)    // нет среднего (все равны)
...
fun findMiddleIf(a: Double, b: Double, c: Double): String {
   ...
   return if ((a > b && a < c) || (a < b && a > c)) {
        "Среднее число: a = $a"
    } else if ((b > a && b < c) || (b < a && b > c)) {
        "Среднее число: b = $b"
    } else {  "Среднее число: c = $c"
    }
}
